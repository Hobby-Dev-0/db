...
extremepro_version = "0.0.8.1"

OAUTH_SCOPE = "https://www.googleapis.com/auth/drive.file"
REDIRECT_URI = "urn:ietf:wg:oauth:2.0:oob"
parent_id = ExtremedB.get("GDRIVE_FOLDER_ID")
G_DRIVE_DIR_MIME_TYPE = "application/vnd.google-apps.folder"

telegraph = Telegraph()
telegraph.create_account(short_name="ExtremeProUserbot Cmds List")
...
UPSTREAM_REPO_URL = "https://github.com/TeamExtremeProUserbot/ExtremeProUserbot"
...
async def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>ExtremeProUserbot {extremepro_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"ExtremeProUserbot {extremepro_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def updater():
    ...
    try:
        ...
        await extremepro_bot.asst.send_message(
            int(ExtremedB.get("LOG_CHANNEL")), f"{txt}\n`directory {error} is not found`"
        )
        repo.__del__()
        return
...
def make_html_telegraph(title, author, text):
    ...
    page = client.post(
        title=title,
        author=author,
        author_url="https://t.me/TeamExtremeProUserbot",
        text=text,
    )
    return page["url"]
...
async def get_user_id(ids):
    if str(ids).isdigit():
        ...
        try:
            userid = (await extremepro_bot.get_entity(ids)).id
...
async def create_token_file(token_file, event):
    flow = OAuth2WebServerFlow(
        ExtremedB.get("GDRIVE_CLIENT_ID"),
        ExtremedB.get("GDRIVE_CLIENT_SECRET"),
        OAUTH_SCOPE,
        redirect_uri=REDIRECT_URI,
    )
    authorize_url = flow.step1_get_authorize_url()
    async with extremepro_bot.asst.conversation(extremepro_bot.uid) as conv:
        await event.edit(
            f"Go to the following link in your browser: [Authorization Link]({authorize_url}) and reply the code",
            link_preview=False,
        )
        response = conv.wait_event(events.NewMessage(from_users=extremepro_bot.uid))
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "Uploaded using ExtremeProUserbot Userbot",
        "mimeType": mime_type,
    }
    if parent_id is not None:
    ...
    try:
        try:
            for i in LOADED[shortname]:
                extremepro_bot.remove_event_handler(i)
            try:
            ...
            for i in reversed(range(len(extremepro_bot._event_builders))):
                ev, cb = extremepro_bot._event_builders[i]
                if cb.__module__ == name:
                    del extremepro_bot._event_builders[i]
                    try:
                        del LOADED[shortname]
                        del LIST[shortname]
                        ADDONS.remove(shortname)
...
async def restart(ult):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        execl(executable, executable, "-m", "Extre")


async def shutdown(ult, dynotype="extremepro"):
...
def ReTrieveFile(input_file_name):
    RMBG_API = ExtremedB.get("RMBG_API")
...
async def get_chatinfo(event):
    ...
    if not chat:
        if event.reply_to_msg_id:
            ...
            chat = event.chat_id
    try:
        chat_info = await extremepro_bot(GetFullChatRequest(chat))
    except BaseException:
        try:
            chat_info = await extremepro_bot(GetFullChannelRequest(chat))
...
async def fetch_info(chat, event):
    chat_obj_info = await extremepro_bot.get_entity(chat.full_chat.id)
    broadcast = (
    ...
    if admins is None:
        try:
            participants_admins = await extremepro_bot(
                GetParticipantsRequest(
            ...
            if "(" not in downloaded_file_name:
                ...
                try:
                    ...
                    if plug in HELP:
                        ...
                        for i in HELP[plug]:
                            output += i
                        output += "\n© @TheExtremeProUserbot"
                        await ok.edit(
                            f"✓ `ExtremeProUserbot - Installed`: `{plug}` ✓\n\n{output}"
                        )
                        ...
                        await ok.edit(f"✓ `ExtremeProUserbot - Installed`: `{plug}` ✓\n\n{kk}")
                        await asyncio.sleep(9)
                        await ok.delete()
                    else:
                        try:
...
async def allcmds(event):
    ...
    t = telegraph.create_page(title="ExtremeProUserbot All Cmds", content=[f"{xx}"])
    w = t["url"]
    await eod(event, f"All ExtremeProUserbot Cmds : [Click Here]({w})", link_preview=False)
...
async def randomchannel(tochat, channel, range1, range2, caption=None):
    ...
    async for x in extremepro_bot.iter_messages(channel, add_offset=do, limit=1):
        try:
            if not caption:
                caption = x.text
            await extremepro_bot.send_message(tochat, caption, file=x.media)
        except BaseException:
            pass
...
async def heroku_logs(event):
    ...
    ok = app.get_log()
    with open("extremepro-heroku.log", "w") as log:
        log.write(ok)
    url = None
    try:
        key = (
    ...
    await extremepro_bot.send_file(
        event.chat_id,
        file="extremepro-heroku.log",
        thumb="resources/extras/extremepro.jpg",
        caption=f"**ExtremeProUserbot Heroku Logs.**\nPasted [here]({url}) too!",
    )
    os.remove("extremepro-heroku.log")
    await xx.delete()


async def def_logs(ult):
    xx = await eor(ult, "`Processing...`")
    with open("extremepro.log") as f:
        ok = f.read()
    url = None
    try:
        key = (
    ...
    await extremepro_bot.send_file(
        ...
        file="extremepro.log",
        thumb="resources/extras/extremepro.jpg",
        caption=f"**ExtremeProUserbot Logs.**\nPasted [here]({url}) too!",
    )
    await xx.edit("Done")
    await xx.delete()
