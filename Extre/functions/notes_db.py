...
def add_note(chat, word, msg, media):
    try:
        ...
        rt = ExtremedB.get("NOTE")
        if not rt:
            the_thing = f"{chat}$|{word}$|{rr}"
            ExtremedB.set("NOTE", the_thing)
        else:
            xx = rt.split("|||")
            for y in xx:
                yy = y.split("$|")
...
def rem_note(chat, word):
    ...
    for i in yx:
        ...
        if str(x[0]) == str(chat) and str(x[1]) == str(word):
            yx.remove(i)
    return ExtremedB.set("NOTE", ls(yx))
